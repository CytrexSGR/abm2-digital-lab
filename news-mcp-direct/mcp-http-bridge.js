 ...
 *
 * Usage: node mcp-http-bridge.js [SERVER_URL]
...
const SERVER_URL = process.argv[2] || process.env.NEWS_MCP_SERVER_URL || 'http://localhost:8001';
const DEBUG = process.env.DEBUG === 'true';

// Always log errors to stderr for Claude Desktop
console.error(`[MCP Bridge] Starting with SERVER_URL: ${SERVER_URL}`);
...
class MCPHttpBridge {
    ...
    async initialize(params) {
        ...
        try {
            ...
            console.error(`[MCP Bridge] Got response status: ${response.status}`);
            ...
            if (!response.data.result || !response.data.result.serverInfo) {
                throw new Error(`Invalid server response: ${JSON.stringify(response.data)}`);
            }

            this.serverInfo = response.data.result.serverInfo;
            console.error(`[MCP Bridge] Connected to ${this.serverInfo.name} v${this.serverInfo.version}`);
            ...
            console.error(`[MCP Bridge] Initialization failed: ${error.message}`);
    ...
    async loadTools() {
        try {
            console.error('[MCP Bridge] Loading tools...');
            const response = await makeHttpRequest('/mcp', JSON.stringify({
                jsonrpc: '2.0',
                id: 2,
                method: 'tools/list',
            ...
            console.error(`[MCP Bridge] Tools response status: ${response.status}`);

            if (response.status === 200 && response.data.result) {
                this.tools = response.data.result.tools || [];
                console.error(`[MCP Bridge] Loaded ${this.tools.length} tools`);
            } else {
                console.error(`[MCP Bridge] Failed to load tools: ${JSON.stringify(response.data)}`);
                this.tools = [];
            }
        } catch (error) {
            console.error(`[MCP Bridge] Failed to load tools: ${error.message}`);
            this.tools = [];
        }
    }
    ...
    start() {
        console.error(`[MCP Bridge] Starting stdio listener...`);
